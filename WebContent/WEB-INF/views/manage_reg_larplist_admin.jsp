...
<title>List Larps and Users Registered for Them</title>
...
<br>
 <h2 style="text-decoration: underline;">List Larps and Users Registered for Them</h2>
 <br>
 <h2>Larp List:</h2>

 <form method="POST" action="ShowUsersInLarpsAdmin"> 
...
<c:forEach items="${larp_list}" var="record">
  <tr>
        		<td><input type="radio" name="chosen_l_id" value="${record.id}" checked/></td>
                <td>${record.id}</td>
...
<button id="submitbutton" type="submit" class="myButton">Get Users In This Game</button> <br> <br>
 </form>
 
<br>
 <h2>User List : ${message}</h2>

 <form method="GET" action="ShowLarpsAndPlayersAdminPanel"> 
 <table class="responstable">
    <tr>
    <th>ID</th>
...
<c:if test="${fn:length(users_in_larp_list) > 0}">
<c:forEach items="${users_in_larp_list}" var="record">
  <tr>
